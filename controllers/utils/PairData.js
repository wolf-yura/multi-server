...
const {bnbPrice, getGlobalData, getBnbPrice}= require('./GlobalData')
...
const  { timeframeOptions } = require('./constants')
// const { useLatestBlock } from './Application'
...
async function getBulkPairData(pairList, bnbPrice) {
  ...
  try {
    ...
    // console.log(current, b1, b2, bWeek);
    ...
    // console.log("--pairs", oneDayResult?.data?.pairs);
 ...
 */
 function useDataForList(pairList) {
 ...
 */
 function usePairTransactions(pairAddress) {
 ...
 function usePairChartData(pairAddress) {
 ...
 */
function useAllPairData() {
  const [state] = usePairDataContext()
  return state || {}
}

async function  getTopPairsData(pairList) {
  const [bnbPrice] = await getBnbPrice();
  console.log(bnbPrice);
  const pairsData= await getBulkPairData(pairList, bnbPrice);  
  return pairsData;
}
module.exports = {
  getTopPairsData
